...
export class AlertErrorComponent implements OnDestroy {
  ...
  constructor(private alertService: AlertService, private eventManager: EventManager, translateService: TranslateService) {
    this.errorListener = eventManager.subscribe('spiritialDigest2App.error', (response: EventWithContent<unknown> | string) => {
      const errorResponse = (response as EventWithContent<AlertError>).content;
      this.addErrorAlert(errorResponse.message, errorResponse.key, errorResponse.params);
    });

    this.httpErrorListener = eventManager.subscribe('spiritialDigest2App.httpError', (response: EventWithContent<unknown> | string) => {
          ...
          if (errorHeader) {
            ...
            for (const fieldError of fieldErrors) {
              ...
              const fieldName: string = translateService.instant(
                `spiritialDigest2App.${fieldError.objectName as string}.${convertedField}`
              );
